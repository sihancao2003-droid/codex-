...
- **ZCode**：通过 ZCode 官方 `billing/balance` 接口读取账户里每个模型（GLM-5.3、GLM-5.3-Flash 等）的 token 额度、已用比例与重置时间；同时监听 `~/.zcode/cli/rollout/model-io-*.jsonl` 会话日志，统计今日已用 tokens（按模型分解），并据此驱动「工作中 / 任务结束」动画，无需任何 ZCode 配置。
...
- 切换到 ZCode 后，单击鲸鱼显示主模型（GLM-5.3）剩余额度百分比，气泡注明「剩余 x / 总量」和当日重置时间；再点气泡依次查看其他模型额度和「今日已用」（含按模型的 token 分解）。
